...
document.addEventListener('keypress', function(e){
    ...
    const rowDuplicated = isDuplicated(x,y,input);
    const colDuplicated = isDuplicated(x,y,input);
    const hasError = rowDuplicated || colDuplicated;

    if(hasError){
        chosenTd.classList.add('error');
    }else{
        chosenTd.classList.remove('errors')
    }
}
});

function isDuplicated(x,y, input){
